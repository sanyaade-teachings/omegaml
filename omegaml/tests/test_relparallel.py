...
class ParallelRelationalMixinTests(OmegaTestMixin, TestCase):
    def setUp(self):
        self.om = Omega()
        self.clean()
    ...
    def test_parallel_process_persist(self):
        ...
        cnx = 'sqlite:///test.db'
        om.datasets.put(cnx, 'largedf', table='largedf', kind=SQLAlchemyBackend.KIND)

        def myfunc(df):
            df['y'] = df['x'] * 2

        om.datasets.put(large, 'largedf', append=False)
        mtab = om.datasets.getl('largedf')
        mtab.transform(myfunc).persist('largedf_transformed', om.datasets)
